...
from models.schemas.graph import GraphState
from utils.graph import dump_messages, prepare_messages

logger = logging.getLogger("lang graph agent")
...
class LangGraphAgent:
    ...
    def __init__(self):
        ...
        logger.info(f"llm_initialized, {settings.LLM_MODEL=}, {settings.ENV=}")
    ...
    async def _get_connection_pool(self) -> AsyncConnectionPool:
        ...
        if self._connection_pool is None:
            try:
                ...
                logger.info(f"connection_pool_created, {max_size=}, {settings.ENV=}")
            except Exception as e:
                logger.error(f"connection_pool_creation_failed, error={str(e)}, {settings.ENV=}")
                # In production, we might want to degrade gracefully
                if settings.ENVIRONMENT == "production":
                    logger.warning(f"continuing_without_connection_pool, {settings.ENV=}")
                    return None
                raise e
    ...
    async def _chat(self, state: GraphState) -> dict:
        ...
        for attempt in range(max_retries):
            try:
                ...
                logger.info(
                    f"{state.session_id=} llm_response_generated, llm_calls_num={llm_calls_num + 1}, {settings.LLM_MODEL}")
                return generated_state
            except OpenAIError as e:
                logger.error(
                    f"llm_call_failed, {llm_calls_num=}, {max_retries=}, attempt={attempt + 1} error={str(e)}"
                )
                llm_calls_num += 1

                # In production, we might want to fall back to a more reliable model
                if settings.ENV == "production" and attempt == max_retries - 2:
                    fallback_model = "gpt-4o"
                    logger.warning(f"using_fallback_model, {fallback_model=}, {settings.ENV=}")
    ...
    async def create_graph(self) -> Optional[CompiledStateGraph]:
        ...
        if self._graph is None:
            try:
                ...
                logger.info(
                    f"graph_created, graph_name={settings.PROJECT_NAME} Agent,has_checkpointer={checkpointer is not None}"
                )
            except Exception as e:
        ...
        try:
            async for token, _ in self._graph.astream(
                ...
                try:
                    ...
                    logger.error(f"{session_id=} Error processing token, error={str(token_error)}")
                    # Continue with next token even if current one fails
                    continue
        except Exception as stream_error:
            logger.error(f"{session_id=} Error in stream processing, error={str(stream_error)}")
            raise stream_error
