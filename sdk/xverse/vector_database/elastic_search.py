...
from xverse.vector_database.utils import format_size, format_timestamp, build_weighted_query
...
class ElasticSearchCore:
    ...
    def _large_batch_insert(self, index_name: str, documents: List[Dict[str, Any]], batch_size: int,
            ...
            for i in range(0, total_docs, batch_size):
                ...
                for j in range(0, len(es_batch), embedding_batch_size):
                    ...
                    try:
                        ...
                        logging.error(
                            f"Embedding API error: {e}, ES batch num: {es_batch_num}, sub-batch start: {j}, size: {len(embedding_sub_batch)}")
                        continue
                ...
                try:
                    ...
                    logging.info(
                        f"Processed ES batch {es_batch_num}/{es_total_batches}, indexed {len(doc_embedding_pairs)} documents.")
    ...
    def get_index_stats(self, index_names: List[str]) -> Dict[str, Dict[str, Dict[str, Any]]]:
        ...
        for index_name in index_names:
            try:
                ...
                process_source = agg_result['aggregations']['process_sources']['buckets'][0]['key'] if \
                    agg_result['aggregations']['process_sources']['buckets'] else ""
                embedding_model = agg_result['aggregations']['embedding_models']['buckets'][0]['key'] if \
                    agg_result['aggregations']['embedding_models']['buckets'] else ""
